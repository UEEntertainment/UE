...
<html lang="en"> 
 <head> 
  ...
  <title>UE Entertainment – Where it all begins</title> 
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"> 
  <style>
    :root{--purple:#7B3CF5;--gold:#FFD700;--dark:#0A0A0A;--white:#FFFFFF;--font:'Segoe UI',sans-serif}
    *{margin:0;padding:0;box-sizing:border-box;font-family:var(--font)}
    body{background:var(--dark);color:var(--white);scroll-behavior:smooth}
    header{height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;background:radial-gradient(circle at 20% 50%,var(--purple) 0%,transparent 40%),radial-gradient(circle at 80% 50%,var(--gold) 0%,transparent 40%);animation:bgMove 20s linear infinite}
    @keyframes bgMove{0%{background-position:0 0}100%{background-position:100% 0}}
    header h1{font-size:3rem;color:var(--gold);margin-bottom:.5rem}
    header p{font-size:1.2rem;margin-bottom:2rem}
    .cta{display:inline-block;margin:.5rem;padding:.8rem 2rem;border:2px solid var(--gold);color:var(--gold);border-radius:30px;text-decoration:none;transition:.3s}
    .cta:hover{background:var(--gold);color:var(--dark)}
    section{padding:80px 20px;max-width:1000px;margin:auto}
    section h2{color:var(--gold);margin-bottom:30px;text-align:center;font-size:2rem}
    .subs{display:grid;gap:30px}
    .card{background:#0f0f0f;border:1px solid var(--purple);border-radius:12px;padding:30px;transition:.3s}
    .card:hover{transform:translateY(-8px)}
    .card h3{color:var(--gold);margin-bottom:10px}
    .timeline{position:relative;border-left:2px solid var(--gold);padding-left:30px;margin-top:20px}
    .timeline li{list-style:none;margin:20px 0;position:relative}
    .timeline li::before{content:'';position:absolute;left:-38px;top:5px;width:12px;height:12px;border-radius:50%;background:var(--gold)}
    .news article{margin-bottom:20px;padding:15px;border-left:3px solid var(--gold)}
    form{display:flex;flex-direction:column;gap:15px;max-width:600px;margin:auto}
    input,textarea,select{padding:10px;border:1px solid var(--purple);background:var(--dark);color:var(--white);border-radius:6px}
    button[type=submit]{align-self:flex-start;padding:10px 25px;background:var(--gold);color:var(--dark);border:none;border-radius:6px;cursor:pointer}
    #lang{position:fixed;top:15px;right:15px;background:var(--dark);border:1px solid var(--gold);border-radius:20px;padding:5px 10px;font-size:.9rem;z-index:1000}
  </style> 
 <style type="text/css" id="dcoder_stylesheet">.hero-carousel{
  max-width:100vw;     /* ancho máximo pantalla */
  max-height:60vh;     /* altura máxima 60 % de la pantalla */
  margin:auto;
  overflow:hidden;
}
.hero-carousel img{
 ...
 <body> <select id="lang"> <option value="en">EN</option> <option value="ko">KO</option> </select> 
  <header> 
   <h1 data-tr="homeTitle">UE Entertainment</h1> <!-- ===== CARRUSEL HERO ===== --> 
...
const dict={
  ...
  ko:{
    ...
    subsTitle:"자회사",
    arcaDesc:"강한 여자들이 태어나는 곳. VIBE와 Pink Phanters 본부.",
